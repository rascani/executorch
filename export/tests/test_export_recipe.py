...
class TestExportRecipeCombine(unittest.TestCase):
    ...
    def test_combine_rejects_two_post_partition_contributors(self) -> None:
        # Sequential composition here is not meaningful: the second backend
        # would rewrite operators the first authored.
        recipes = [
            ExportRecipe(
                name=n,
                lowering_recipe=self._lowering(op_backends=[_StubOpBackend()]),
            )
            for n in ("a", "b")
        ]
        with self.assertRaisesRegex(ValueError, "op_backends"):
            ExportRecipe.combine(recipes)
