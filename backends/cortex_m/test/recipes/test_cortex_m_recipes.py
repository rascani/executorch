...
_BACKEND_LOGGER = "executorch.backends.cortex_m.op_backend"
...
class TestCortexMRecipeRegistration(unittest.TestCase):
    def test_backend_registered(self) -> None:
        self.assertIn(CORTEX_M_BACKEND, recipe_registry.list_backends())

    def test_supported_recipes_match_enum(self) -> None:
        supported = recipe_registry.get_supported_recipes(CORTEX_M_BACKEND)
        self.assertEqual(set(supported), set(CortexMRecipeType))
    ...
    def test_unknown_recipe_returns_none(self) -> None:
        ...
        self.assertIsNone(CortexMRecipeProvider().create_recipe(_StubRecipeType.FOO))


class TestCortexMRecipeConstruction(unittest.TestCase):
    ...
    def test_target_selects_cpu(self) -> None:
        for target, expected_cpu in (
            ("cortex-m33", CortexM.M33),
            ("cortex-m55", CortexM.M55),
            ("cortex-m85", CortexM.M85),
        ):
            with self.subTest(target=target):
                recipe = ExportRecipe.get_recipe(CortexMRecipeType.INT8, target=target)
                self.assertEqual(_target_config(recipe).cpu, expected_cpu)

    def test_default_target_is_m55(self) -> None:
        recipe = ExportRecipe.get_recipe(CortexMRecipeType.INT8)
        self.assertEqual(_target_config(recipe).cpu, CortexM.M55)
    ...
    def test_isa_unsupported_on_cpu_raises(self) -> None:
        with self.assertRaises(ValueError):
            ExportRecipe.get_recipe(
                CortexMRecipeType.INT8, target="cortex-m0", isa=cmsis_nn.Backend.MVE
            )

    def test_unknown_target_raises(self) -> None:
        with self.assertRaises(ValueError):
            ExportRecipe.get_recipe(CortexMRecipeType.INT8, target="cortex-a53")
...
class TestCortexMRecipeExport(unittest.TestCase):
    def test_channels_last_input_lowers_to_cmsis_kernels(self) -> None:
        with self.assertNoLogs(_BACKEND_LOGGER, level="INFO"):
            program = _export(
                # Passing a documented kwarg must not warn about it either.
...
class TestCortexMRecipeTargetReachesLowering(unittest.TestCase):
...
class TestCortexMLoweringLeavesItsInputAlone(unittest.TestCase):
...
class TestCortexMRecipeMultiMethod(unittest.TestCase):
