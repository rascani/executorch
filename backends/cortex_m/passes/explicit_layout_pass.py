...
_QPARAM_META_KEYS = ("input_qparams", "output_qparams", "custom")
...
def _has_input_and_output_qparams(node: torch.fx.Node) -> bool:
    return _has_input_qparams(node) and bool(node.meta.get("output_qparams"))
...
def _supports_max_pool2d(node: torch.fx.Node) -> bool:
    if not _is_rank4(node) or not _has_input_qparams(node):
        return False
    if (
...
class CortexMExplicitLayoutPass(ToContiguousChannelsLastPass):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            preserve_meta_keys=_QPARAM_META_KEYS,
            strict=strict,
        )
