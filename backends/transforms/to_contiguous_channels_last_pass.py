...
class ToContiguousChannelsLastPass(ExportPass):
    ...
    def call(self, graph_module: torch.fx.GraphModule) -> PassResult:
        ...
        replacement = ReplaceOpsWithChannelsLastVariants(
            ...
            op_map=self.op_map,
            preserve_meta_keys=self.preserve_meta_keys,
        ).call(graph_module)
        ...
        if self.strict and (
            self.report.internal_copy_count or self.report.unknown_copy_count
        ):
            raise RuntimeError(
                "Channels-last layout optimization left "
                f"{self.report.internal_copy_count} internal and "
                f"{self.report.unknown_copy_count} unknown copies. "
